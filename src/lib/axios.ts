...
import { BASE_API_ENDPOINT } from "@/lib/constants";
  ...
  private _setInterceptors() {
    this.instance.interceptors.request.use((config) => {
      ...
      if (state?.tokens?.accessToken) {
        config.headers.Authorization = `Token ${state.tokens.accessToken}`;
      }
      return config;
    });
...
export const mainClient = new AxiosClient({ baseURL: "" }).client;
